...
// dynamically.
__webpack_require__.p = document.querySelector('body').getAttribute('data-base-url') + 'nbextensions/ipyevents/';
...
var common_event_message_names = [
    'altKey',
    'ctrlKey',
    'metaKey',
    'shiftKey',
...
var mouse_standard_event_message_names = [
    'button',
    'buttons',
    'clientX',
    'clientY',
...
var mouse_added_event_message_names = [
    'dataX',
    'dataY',
    'relativeX',
    'relativeY',
...
var wheel_standard_event_names = [
    'deltaX',
    'deltaY',
    'deltaZ',
    'deltaMode'
];
var drag_standard_event_names = [
    'dataTransfer'
];
var key_standard_event_names = [
    'code',
    'key',
    'location',
    'repeat'
...
var EventModel = /** @class */ (function (_super) {
    ...
    EventModel.prototype.defaults = function () {
        return _.extend(_super.prototype.defaults.call(this), {
            ...
            _model_module_version: version_1.EXTENSION_SPEC_VERSION,
            source: null,
            watched_events: [],
    ...
    EventModel.prototype.key_or_mouse = function (event_type) {
    ...
    EventModel.prototype._cache_listeners = function (event_type, view, handler) {
    ...
    EventModel.prototype.update_listeners = function () {
        // Remove all existing DOM event listeners
        this.remove_listeners();
        // Add watchers to any existing views of the model
        this.attach_listeners();
    };
    EventModel.prototype.remove_listeners = function () {
        // Remove all of the event listeners stored in the cache.
        if (listener_cache[this.model_id]) {
            for (var _i = 0, _a = listener_cache[this.model_id]; _i < _a.length; _i++) {
                var listener = _a[_i];
    ...
    EventModel.prototype._add_listeners_to_view = function (view) {
        // Add listeners for each of the watched events
        for (var _i = 0, _a = this.get('watched_events'); _i < _a.length; _i++) {
            var event_1 = _a[_i];
            switch (this.key_or_mouse(event_1)) {
                case "keyboard":
                    this._add_key_listener(event_1, view);
                    break;
                case "mouse":
                    var prevent_default = this.get('prevent_default_action');
                    var handler = this._dom_click.bind(this, view);
                    view.el.addEventListener(event_1, handler);
                    // Keep track of the listeners we are attaching so that we can
                    // remove them if needed.
                    this._cache_listeners(event_1, view, handler);
                    break;
                default:
        ...
        if (this.get('xy_coordinate_system')) {
            var prevent_default = this.get('prevent_default_action');
            var handler = this._set_xy.bind(this, view);
            var event_2 = 'mousemove';
            view.el.addEventListener(event_2, handler);
            this._cache_listeners(event_2, view, handler);
        }
    };
    EventModel.prototype.attach_listeners = function () {
        var _this = this;
        var current_source = this.get('source');
        _.each(current_source.views, function (view_promise) {
            Promise.resolve(view_promise).then(function (view) {
                _this._add_listeners_to_view(view);
            });
        });
    };
    EventModel.prototype._add_key_listener = function (event_type, view) {
        ...
        var key_handler = function (event) {
            ...
            if (_this.get('ignore_modifier_key_events') &&
                _.contains(_this.get('_modifier_keys'), event.key)) {
                // If the key event is supposed to be ignored, then skip it.
                return;
            }
            _this._send_dom_event(event);
        ...
        var disable_key_listen = function () {
    ...
    EventModel.prototype._supplement_mouse_positions = function (generating_view, event) {
        // Get coordinates relative to the container
        var relative_xy = _get_position(generating_view, event);
        event['relativeX'] = relative_xy.x;
        event['relativeY'] = relative_xy.y;
        if ('_data_xy' in generating_view) {
            var data_coords = generating_view['_data_xy'](event);
            ...
            // view.
            var data_coords = _click_location_original_image(generating_view, event);
            event['dataX'] = data_coords.x;
            event['dataY'] = data_coords.y;
        }
    ...
    EventModel.prototype._dom_click = function (generating_view, event) {
    ...
    EventModel.prototype._set_xy = function (generating_view, event) {
        // Get coordinates relative to the container, and
        // data (i.e. "natural") coordinates.
        this._supplement_mouse_positions(generating_view, event);
        var coord_type = this.get('xy_coordinate_system');
        var coords = [event[coord_type + 'X'], event[coord_type + 'Y']];
    ...
    EventModel.prototype._send_dom_event = function (event) {
        ...
        for (var _i = 0, message_names_1 = message_names; _i < message_names_1.length; _i++) {
            var i = message_names_1[_i];
            event_message[i] = event[i];
        }
        event_message['event'] = event['type'];
        this.send(event_message, {});
    };
    EventModel.serializers = __assign({}, base_1.WidgetModel.serializers, { source: { deserialize: base_1.unpack_models } });
    return EventModel;
}(base_1.WidgetModel));
 ...
 */
exports.EXTENSION_SPEC_VERSION = '1.4.1';
